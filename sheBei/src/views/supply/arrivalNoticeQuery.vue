    ...
    <el-card class="searchCard">
      <div style="margin-top: 10px; max-width: 1600px">
        <el-form label-width="110px" :inline="true" class="demo-form-inline">
          ...
          <el-form-item class="labelWidth" label="接收时间：">
            <div class="block">
              <el-date-picker
          ...
          <el-form-item class="formRight" label="确认人：">
            <el-input
              v-model="queryData.confirmedMan"
              class="inputWidth"
              prefix-icon="el-icon-search"
              placeholder="请输入确认人名称"
            ></el-input>
          </el-form-item>
          <el-form-item class="labelWidth" label="确认时间：">
            <div class="block">
              <el-date-picker
                :default-time="['00:00:00', '23:59:59']"
                value-format="yyyy-MM-dd HH:mm:ss"
...
export default {
  ...
  methods: {
    timeChange(str) {
      if (str === "接收时间") {
        if (this.receiptTime != null && this.receiptTime.length > 0) {
          this.queryData.receivedAtStart = this.receiptTime[0];
          this.queryData.receivedAtEnd = this.receiptTime[1];
        } else {
          this.queryData.receivedAtStart = "";
          this.queryData.receivedAtEnd = "";
        }
      } else if (str === "确认时间") {
        if (this.sureTime != null && this.sureTime.length > 0) {
          this.queryData.confirmedAtStart = this.sureTime[0];
          this.queryData.confirmedAtEnd = this.sureTime[1];
        } else {
          this.queryData.confirmedAtStart = "";
          this.queryData.confirmedAtEnd = "";
        }
      }
    },
    clickOrder(id) {
    ...
    getMenuList(str) {
      ...
      supplyApi.getArrivalNoticeQuery(this.queryData).then(res=>{
        if(res.data.code===200){
