  ...
  <div class="arrivalNoticeQueryMain">
    <el-card class="searchCard">
      <div style="margin-top: 10px; max-width: 1600px">
        <el-form label-width="110px" :inline="true" class="demo-form-inline">
          ...
          <el-form-item class="formRight" label="生产订单号：">
            <el-input
              v-model="queryData.productionOrderNumber"
              class="inputWidth"
              prefix-icon="el-icon-search"
              placeholder="请输入关键字搜索"
            ></el-input>
    ...
    <el-card class="contentCard">
      <div class="tableMainQin">
              ...
              <div class="orderNumber">
                <div class="titleContentleft">
                  <div class="titleContent">
                    ...
                    <div>
                      ...
                      <span
                        :title="
                          item.linkmanName
                            ? item.linkmanName + ' ' + item.linkmanPhone
                            : ''
                        "
                        >{{
                          item.linkmanName
                  ...
                  <div class="titleContent">
                    ...
                    <div v-if="item.status === 3 || item.status === 4">
                      <span>拒绝人：</span>
                      <span :title="item.refusedOperatorName">{{
                        item.refusedOperatorName
                      }}</span>
                    </div>
                    <div v-if="item.status === 3 || item.status === 4">
                      <span>拒绝原因：</span>
                      <span :title="item.refuseReason">{{
                        item.refuseReason
                      }}</span>
                    </div>
                    <div v-if="item.status === 5">
                      <span>确认时间：</span>
                      <span :title="item.confirmedAt">{{
                        item.confirmedAt
                      }}</span>
                    </div>
                    <div v-if="item.status === 5">
                      <span>确认人：</span>
                      <span :title="item.confirmedOperatorName">{{
                        item.confirmedOperatorName
                      }}</span>
                    </div>
                    ...
                    <div v-if="item.status === 7">
                      <span>中止人：</span>
                      <span :title="item.abortOperatorName">{{
                        item.abortOperatorName
                      }}</span>
                    </div>
                    <div v-if="item.status === 7">
                      <span>中止原因：</span>
                      <span :title="item.abortReason">{{
                        item.abortReason
                      }}</span>
                    </div>
        ...
        <div v-show="tableList.length < 1" class="none">暂无数据</div>
      </div>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    getMenuList(str) {
      ...
      buyerApi.getOrderQuery(this.queryData).then((res) => {
        if (res.data.code === 200) {
