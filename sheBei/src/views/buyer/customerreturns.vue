    ...
    <el-card class="searchCard">
      <div style="margin-top: 10px">
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item class="labelWidth" label="退货日期：">
            <div class="block">
              <el-date-picker
...
import returnDetails from "@/components/cargoStatistics/returnDetails";
  ...
  methods: {
    ...
    timeChange() {
      if (this.timeValue != null && this.timeValue.length > 0) {
        ...
        this.searchContent.rejectedTimeEnd = this.timeValue[1];
      } else {
        this.searchContent.receivedAtStart = "";
        this.searchContent.rejectedTimeEnd = "";
      }
    },
    // 表格数据列表查询
    getList() {
      buyerApi.buyerrejected(this.searchContent).then(({ data: res }) => {
        if (res.code == 200) {
    ...
    //导出
    exportExcel() {
      buyerApi.buyerrejectedexport(this.searchContent).then(({ data:res }) => {
        if (this.list.length<1) {
          ...
          createA.href = href;
          createA.download = "退货统计" + this.getData() + ".xls";
    ...
    detailPage(val) {
      ...
      supplyApi.suppliercargo(this.myParams).then(({ data: res }) => {
        if (res.code == 200) {
          this.detailList = {};
