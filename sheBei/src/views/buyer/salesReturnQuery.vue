<template>
  <div class="salesReturnQueryMain">
      ...
      <div style="margin-top: 10px; max-width: 1600px">
        <el-form label-width="110px" :inline="true" class="demo-form-inline">
          <el-form-item class="formRight" label="退货单号：">
            <el-input
              v-model="queryData.rejectedCode"
              class="inputWidth"
          ...
          <!-- <el-form-item class="formRight" label="供应商名称：">
            <el-input
              v-model="queryData.supplierCompanyName"
              class="inputWidth"
              prefix-icon="el-icon-search"
          ...
          <el-form-item class="formRight" label="供应商名称：">
            <el-select
                v-model="queryData.supplierCompanyId" 
          ...
          <el-form-item class="labelWidth" label="退货时间：">
            <div class="block">
              <el-date-picker
              ...
              <div class="orderNumber">
                <div class="titleContentleft">
                  <div class="titleContent">
                    ...
                    <div>
                      <span>退货单号：</span>
                      <span :title="item.rejectedCode">{{ item.rejectedCode }}</span>
                    </div>
                    <div>
                      <span>退货时间：</span>
                      <span :title="item.rejectedTime">{{ item.rejectedTime }}</span>
                    </div>
                    <div>
                      <span>交货日期：</span>
                      <span :title="item.deliverTime">{{ item.deliverTime }}</span>
                    </div>
                    <div>
                      <span>供应商名称：</span>
                      <span :title="item.supplierCompanyName">{{ item.supplierCompanyName }}</span>
                    </div>
                    <div>
                      <span>联系人：</span>
...
<script>
import {buyerApi,supplyApi} from "../../api/user.js"
  ...
  methods: {
    timeChange(str) {
        ...
        if (this.receiptTime != null && this.receiptTime.length > 0) {
          this.queryData.rejectedTimeStart = this.receiptTime[0];
          this.queryData.rejectedTimeEnd = this.receiptTime[1];
        } else {
          this.queryData.rejectedTimeStart = "";
          this.queryData.rejectedTimeEnd = "";
        }
      } else if (str === "确认时间") {
        if (this.sureTime != null && this.sureTime.length > 0) {
          this.queryData.deliverTimeStart = this.sureTime[0];
          this.queryData.deliverTimeEnd = this.sureTime[1];
        } else {
          this.queryData.deliverTimeStart = "";
          this.queryData.deliverTimeEnd = "";
        }
      }
    },
    clickOrder(id) {
    ...
    orderStatus(val) {
      let status = {};
	    if (val === -1) {
        status = { name: "待下发", color: "#452DB3" };
      }else if (val === 0) {
        status = { name: "待确认", color: "#F95E01" };
      }else if (val ===1) {
        status = { name: "已确认", color: "#3C89FF" };
      } else if (val ===2) {
        status = { name: "已拒绝", color: "#4017EF" };
      }else if (val ===3) {
        status = { name: "已完成", color: "#3AA72D" };
      }
      return status;
    },
    //查询
    getMenuList(str) {
      ...
      this.queryData.entranceMark='BUYER'
      buyerApi.getSalesReturnQuery(this.queryData).then(res=>{
        if(res.data.code===200){
