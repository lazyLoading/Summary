<template>
  <div class="arrivalNoticeQueryMain">
              ...
              <div class="orderNumber">
                <div class="titleContentleft">
                  <div class="titleContent">
                    ...
                    <div>
                      <span>收货联系人：</span>
                      <span :title="item.receiveUserName?item.receiveUserName+' '+item.receiveUserPhone:''">{{ item.receiveUserName?item.receiveUserName+' '+item.receiveUserPhone:'' }}</span>
                    </div>
                  </div>
                  <div class="titleContent">
                    <div>
                      ...
                      <span :title="item.deliveryAddress">{{ item.deliveryAddress }}</span>
					          ...
					          <div>
                      <span>到货日期：</span>
                      <span :title="item.requirementAt">{{ item.requirementAt }}</span>
                    </div>
    ...
    >
      ...
      <!-- <OrderDetail :detailData="detailData" :infoLabel="infoLabel">
      </OrderDetail> -->
...
export default {
  ...
  methods: {
    timeChange(str) {
      if (str === "接收时间") {
        if (this.receiptTime != null && this.receiptTime.length > 0) {
          this.queryData.issuedAtStart	 = this.receiptTime[0];
          this.queryData.issuedAtEnd = this.receiptTime[1];
        } else {
        ...
        if (this.sureTime != null && this.sureTime.length > 0) {
          this.queryData.deliveredAtStart = this.sureTime[0];
          this.queryData.deliveredAtEnd = this.sureTime[1];
        } else {
   ...
   clickOrder(e) {
      ...
      this.statusName = this.orderStatus(e.status).name
      if (e.status == 3 ) {
        this.statusTag = "";
      } else if (e.status == 2) {
    ...
    getMenuList(str) {
      ...
      buyerApi.getArrivalNoticeQuery(this.queryData).then(res=>{
        if(res.data.code===200){
