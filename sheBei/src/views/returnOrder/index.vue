  ...
  <div class="statementquery">
          ...
          <el-form-item class="formRight" label="退货时间：">
            <div class="block">
              <el-date-picker
                  ...
                  @change="timeChange('rejectedTime')"
                  align="right"
              ></el-date-picker>
            </div>
          </el-form-item>
          ...
          <el-form-item class="formRight" label="交货日期：">
            <div class="block">
              <el-date-picker
                  ...
                  @change="timeChange('receivedTime')"
                  align="right"
              ></el-date-picker>
            </div>
          </el-form-item>
    ...
    <el-card class="contentCard">
      <div class="view_operation">
        ...
        <div class="flex"></div>
        <el-button type="primary" @click="handleCreate">新增退货单</el-button>
        ...
        <el-upload
            name="rejected-file"
            :action="expImportReturnOrder"
            :headers="headers"
            :file-list="fileList"
            :show-file-list="false"
            :on-success="handleSuccess"
            :on-change="handleChange1">
          <el-button class="margin-10" type="primary">导入退货单</el-button>
        </el-upload>
            ...
            <el-collapse-item v-for="(item, i) in pageInfo.content" :key="i">
              ...
              <div class="w-full">
                ...
                <div v-if="item.status === -1" class="w-full text-right m-t-20">
                  ...
                  <el-button type="primary" @click="showDialog('delete', item)">删除</el-button>
                </div>
                <div v-if="item.status === 0" class="w-full text-right m-t-20">
                  <el-button type="primary" @click="handleBack(item)">撤回</el-button>
                  <el-button type="primary" @click="showDialog('delete', item)">删除</el-button>
                </div>
                <div v-if="item.status === 2" class="w-full text-right m-t-20">
                  <el-button type="primary" @click="handleDispatch(item)">下发</el-button>
                  <el-button type="primary" @click="handleEdit(item)">编辑</el-button>
                </div>
              </div>
      ...
      <div class="dialog_container">
        <div class="flex-row items-center justify-center w-full">
          <el-image
              ...
              fit="fill"></el-image>
          <div class="m-l-15 f-s-16">{{dialogType === 0? '退货单下发成功!' : dialogType === 1? '退货单撤回成功!' : '退货单删除成功!'}}</div>
        </div>
        <div class="dialog_footer">
    ...
    <el-drawer
      ...
      <template slot="title">
        <div class="drawer-title">
          <strong>{{type === 'create' ? '新增退货单' : '编辑退货单'}}</strong>
        </div>
      </template>
...
import {
  ...
  getReturnDelete,
  getDown,
  expImportReturnOrder,
  getReCheckStatus
} from '@/api/arrivalNotice'
import operatorBill from "@views/returnOrder/components/operatorBill.vue";
...
export default {
  ...
  methods: {
    ...
    getCheckStatus (type, event) {
      const _self = this
      getReCheckStatus ({id: event.id, currentStatus: event.status}).then(res => {
    ...
    handleDownLoad () {
      const _self = this
      getDown({fileName: '退货单导入模板.xlsx'}).then(res => {
        console.log(res)
        if (res.data.code === 200) {
    ...
    handleDelete () {
      if (this.btnType === 'delete') {
        this.getCheckStatus('delete',this.myevent)
      } else {
        this.handleDelete2()
      }
    },
    ...
    timeChange(event) {
      if (event === 'rejectedTime') {
        this.searchParams.deliverTimeStart =  this.rejectedTime ? this.rejectedTime[0] : ''
        this.searchParams.deliverTimeEnd =  this.rejectedTime ? this.rejectedTime[1] : ''
      } else {
        this.searchParams.deliverTimeStart =  this.receivedTime ? this.receivedTime[0] : ''
        this.searchParams.deliverTimeEnd =  this.receivedTime ? this.receivedTime[1] :  ''
      }
    },
    clickOrder(e) {
    ...
    orderStatus(val) {
      let status = {};
      if (val == -1) {
        status = { name: "待下发", color: "#F95E00" };
      } else if (val == 0) {
        status = { name: "待确认", color: "#3183FF" };
      } else if (val == 1) {
    ...
    updateDispatchItem(idList, index) {
      let _self = this
      this.dialogType = index
      updateReturnItem(idList, 0).then(res => {
        console.log(res)
        if (res.data.code === 200) {
          _self.checkedCities = []
          _self.clearSearch()
    ...
    getDelete(idList, index) {
      let _self = this
      this.dialogType = index
      getReturnDelete(idList).then(res => {
        if (res.data.code === 200) {
          _self.checkedCities = []
          _self.clearSearch()
